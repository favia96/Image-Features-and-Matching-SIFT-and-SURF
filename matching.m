...
%title('SIFT Keypoints on Reference and Target');
%saveas(figure1,'sift_keypoints_ref_tar.png');


%% 'fixed threshold' matching algorithm
%%euclidean distance on feature (descriptor) space below a fixed thresh
% fixed_thresh_euc = 62.5; %optimal %trial-error (min=50.52, mean=99.04, median=98.39, max=141.57) if euclidean
% with 64 as suboptimal
...
% end
% 
% title('SIFT Match: Fixed Threshold = 64 (Suboptimal)');
% legend('Reference','Target');
% saveas(figure1,'match_sift_fixed_thresh_suboptimal.png');
...
% %    [m,n] = min(chi_square_dist_sift(l,:));  
...
% ratio_thresh = 0.89; %optimal through trial and error, %in literature the best is 0.8
% %with 0.91 as a suboptimal
% 
% for i = 1 : n_det_ref
...
% end
% 
% title('SIFT Match: Nearest Neighbour Distance Ratio - Thresh = 0.91 (suboptimal)');
% legend('Reference','Target');
% saveas(figure1,'match_sift_nearest_neighbour_dist_ratio_suboptimal.png');
...
title('SURF Match: Nearest Neighbour Distance Ratio - Thresh = 0.87 (suboptimal)');
saveas(figure1,'match_surf_nearest_neighbour_dist_ratio_suboptimal.png');
